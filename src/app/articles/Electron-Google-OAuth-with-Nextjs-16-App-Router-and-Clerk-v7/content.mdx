...
```text
Electron login button
  -> shell.openExternal("https://app.example.com/oauth/start?provider=google&state=...")
  -> /oauth/start calls Clerk signIn.authenticateWithRedirect()
  -> Clerk redirects to Google
  -> Google redirects back to Clerk
  -> Clerk redirects to /oauth/sso-callback
  -> <AuthenticateWithRedirectCallback /> completes the browser-side flow
  -> Clerk redirects to /oauth/callback
  -> /oauth/callback calls /api/oauth/create-signin-token
  -> Browser redirects to corelive://oauth/callback?state=...&token=...
  -> Electron receives the deep link
  -> Electron renderer exchanges the token with Clerk's ticket strategy
  -> Electron activates its own Clerk session
```
